...
public class Proj1{
	...
	private Text word = new Text();
        ...
        @Override
           ...
           throws IOException, InterruptedException {
               ...
               Func func = funcFromNum(funcNum);
                // YOUR CODE HERE
	 	Matcher target = WORD_PATTERN.matcher(docContents.toString());
		ArrayList<Integer> targetIndices = new ArrayList<Integer>();
		int index = 0;
		while (target.find())	
 		{
                	word.set(target.group().toLowerCase());
		...
		while (matcher.find())
		{
			...
			if (!((word.toString()).equals(targetGram.toLowerCase())))
			{
				for (int i = 0; i < targetIndices.size(); i++)	
				{		
				distance = Math.abs(index2 - targetIndices.get(i));
				minDistance = Math.min(distance, minDistance);	
				}
				context.write(word,new DoublePair(1,func.f(minDistance)));	
    ...
    public static class Combine1 extends Reducer<Text, DoublePair, Text, DoublePair> {
		...
		for (DoublePair value : values)
		{
			...
			sumOfF += value.getDouble2();
		}
		context.write(key, new DoublePair(sum,sumOfF)); 
            }
    }
    ...
    public static class Reduce1 extends Reducer<Text, DoublePair, DoubleWritable, Text> {
		...
		if (sumOfF > 0)
		{
			cooccurrence = sumOfF * Math.pow(Math.log(sumOfF),3) / sum;
			context.write(new DoubleWritable(-1*cooccurrence), key);
		}
		else
		{
    ...
    public static class Reduce2 extends Reducer<DoubleWritable, Text, DoubleWritable, Text> {
		...
		for (Text value : values)
		{
			if (n < N_TO_OUTPUT)
			{
                		context.write(new DoubleWritable(key.get()*-1+0),value);
				n++;
			}
           	}
            }
